...
def general_setup():
    ...
    yield path, bucket, key, pii_fields, input_dict





def test_calls_internal_functions_correctly(general_setup):
    path, bucket, key, pii_fields, input_dict = general_setup

    mock_deal_with_csv = Mock(return_value="mocked_byte_stream")
    with patch('obf_lib.main.return_data_type') as mock_rdt, \
